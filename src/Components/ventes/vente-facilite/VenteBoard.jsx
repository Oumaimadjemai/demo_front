...
export default function VenteBoardFacilite({ setActiveComponent }) {
  ...
  const fetchVente = (search = searchTerm, appliedFilters = filters) => {
    ...
    if (fromDate && toDate && dayjs(fromDate).isAfter(dayjs(toDate))) {
      [fromDate, toDate] = [toDate, fromDate]; // swap
    }
  ...
  const handleDetailsClick = async (vente) => {
    try {
      const res = await axios.get(`vente/ventes-facilite/${vente.id}/`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access_token")}`,
        },
      });
        ...
        {!VenteDetail?.lignes_detail?.length ? (
          ...
          <DetailTable
            rows={VenteDetail.lignes_detail.map((item) => ({
              ...
              prix_vente_facilite: parseFloat(item.prix_unitaire),
              quantite: item.quantite,
              magasin: item.produit_detail.magasin_detail?.nom,
